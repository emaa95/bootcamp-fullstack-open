...
const App = () => {
  ...
  const addBlog = async (BlogToAdd) => {
    try {
      ...
      const createBlog = await blogService.create(BlogToAdd)
  ...
  const removeBlog = async (id) => {
    ...
    try {
      if (window.confirm(`Delete ${blogToRemove.title}?`)) {
        ...
        setSuccessMessage(`Blog ${blogToRemove.title} was successfully deleted`)
        setErrorMessage(null)
        setTimeout(() => {
          setSuccessMessage(null)
        }, 5000)
      ...
      setErrorMessage(`Cannot delete blog ${blogToRemove.title}`)
      setSuccessMessage(null)
      setTimeout(() => {
        setErrorMessage(null)
      }, 5000)
